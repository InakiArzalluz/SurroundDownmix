...
algoritmoDownix1 = '\"pan=stereo|c0 < 1.0*c0 + 0.707*c2 + 0.707*c4|c1 < 1.0*c1 + 0.707*c2 + 0.707*c5\"'

# ATSC with volume boost
algoritmoDownix2 = '\"volume=2.0,pan=stereo|c0 < 1.0*c0 + 0.707*c2 + 0.707*c4|c1 < 1.0*c1 + 0.707*c2 + 0.707*c5\"'

# without discarding the LFE channel:
algoritmoDownix3 = '\"pan=stereo|c0=0.5*c2+0.707*c0+0.707*c4+0.5*c3|c1=0.5*c2+0.707*c1+0.707*c5+0.5*c3\"'

# Nightmode:
algoritmoDownix4 = '\"pan=stereo|c0=c2+0.30*c0+0.30*c4|c1=c2+0.30*c1+0.30*c5\"'

algoritmosDownix = (algoritmoDownix1, algoritmoDownix2, algoritmoDownix3, algoritmoDownix4)
# ---------------------------------------------------------------------------------------------------------------
...
def downmix(demuxLocation, dict_dict_stream, algorithm):
    print("Downmixing")
    ...
    for filename2 in os.listdir(demuxLocation):
        ...
        if os.path.isfile(filepath) and filename2.find('_downmix') == -1 and dict_dict_stream[filepath]['channels'] == '6':
            ...
            downmixCommand = f'{ffmpeg} -y -i \"{filepath}\" -c {codec} {aac_fast} -af {algoritmosDownix[algorithm]} \"{outputFileNoExt}_downmix.{codec}\"'
    ...
    for root, dirs, files in os.walk(dir_inputs):
        for filename in files:
            ...
            if filename.rsplit(sep='.')[-1] == 'mkv':
                ...
                downmix(demuxLocation, dict_dict_stream, algor)
                inputFile = os.path.join(root, filename)
